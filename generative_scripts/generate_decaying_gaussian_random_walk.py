...
import pandas as pd; import os; from scipy import stats; from glob import glob; import argparse

parser = argparse.ArgumentParser()
parser.add_argument("--initial_mu_arm1", type=int)
parser.add_argument("--initial_mu_arm2", type=int)
...
initial_mu_arm1, initial_mu_arm2 = args.initial_mu_arm1, args.initial_mu_arm2

n_bandit_arms = 2
trial_duration = 1.5

n_runs = 2
max_run_duration = 8.99 # in m, allowing 5 s for HRF after final trial

n_trials = 600
final_n_trials = n_trials // 10
...
for ses in range(n_sessions):
    ...
    while (run_duration > max_run_duration):  # make sure values are positive
        ...
        for run in np.arange(n_runs)+1:
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(home, 'experimental_parameters/reward_schedule_figs/sub{}_ses{}_reward_schedule_diffusion{}_theta{}_decay{}_run{}.png').format(sub, ses_one_idx, diffusion_noise_sigma, theta, decay_parameter, run))
            # plt.show()
            ...
            # plt.vlines(x=trials[::theta], ymin=point_range.min(), ymax=point_range.max())
            plt.tight_layout()
            plt.savefig(os.path.join(home, 'experimental_parameters/reward_schedule_figs/sub{}_ses{}_delta_reward_schedule_diffusion{}_theta{}_decay{}_run{}.png').format(sub, ses_one_idx, diffusion_noise_sigma, theta, decay_parameter, run))
            # plt.show()

            all_bandit_arm_points_df_unmelted.to_csv(os.path.join(home,
            'experimental_parameters/reward_parameters/sub{}_ses{}_diffusive_bandit_run{}.csv').format(sub, ses_one_idx, run),
            index=False)
